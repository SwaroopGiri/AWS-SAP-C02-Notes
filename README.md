...
Amazon Workspaces and AppStream are similar but different. Workspaces are desktop as a service. AppStream is Desktop Application hosting on web UI and streaming service, clients can access these applications via web browser and work on them.
...
Enforcing standardized tagging can be done via aws config rules or custom scripts. e.g EC2 instances not properly tagged are stopped or terminated
...
You can upload all SSL certificates of different domains in the ALB using the console and bind multiple certificates to the same secure listener on your load balancer. ALB will automatically choose the optimal TLS certificate for each client using Server Name Indication (SNI)
...
Snowball Edge's onboard S3-Compatible Endpoint makes for seamless transfer from a data warehouse S3 interface, which most of the major data warehouse vendors support. Hadoop data can be copied to the S3 endpoint after mounting it on a staging workstation. 
...
Endpoint services owned by your organization in separate vpc can leverage private link
...
An Application Load Balancer cannot be assigned an Elastic IP address (static IP address), use ALB with Global Accelerator instead
Network Load Balancer supports Elastic IP
